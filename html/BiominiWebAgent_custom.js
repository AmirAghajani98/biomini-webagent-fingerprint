...
function IsIEbrowser() {
  ...
  if (
    (browser.indexOf("chrome") != -1 || browser.indexOf("firefox") != -1) &&
...
function AddScannerList(ScannerInfos) {
  ...
  $.each(ScannerInfos, function (key) {
    strBuffer =
      "[" +
      ScannerInfos[key].DeviceIndex +
      "]" +
      ScannerInfos[key].DeviceType +
      " (" +
      ScannerInfos[key].ScannerName +
      ")";
    $("#slt_ScannerList").append(
      "<option value='" +
        key +
        "' class='ScannerListOptions' >" +
        strBuffer +
        "</option>",
    );
    count = key;
  ...
  jQuery.ajax({
    ...
    success: function (msg) {
      ...
      if (msg.retValue == 0) {
        ...
        $("#DDb_TimeoutOpt > option[value=" + msg.timeout / 1000 + "]").attr(
          "selected",
          "true",
        );
        $("#DDb_SecuLevOpt > option[value=" + msg.securitylevel + "]").attr(
          "selected",
          "true",
        );
        $("#DDb_Tpltype > option[value=" + msg.TemplateType + "]").attr(
          "selected",
          "true",
        );
        $("#DDb_FakeLevOpt > option[value=" + msg.fakeLevel + "]").attr(
          "selected",
          "true",
        );
        $("#cb_DetectFakeAdvancedMode").attr(
          "checked",
          msg.detectFakeAdvancedMode,
        );
      }
    },
    error: function (request, status, error) {
...
function DisplayScannerList() {
  ...
  document.getElementById("Tb_DisplayLog").value = "";
  var StateString =
    document.getElementById("DDb_Scanum").value + ":" + strBuffer;
...
function PreviewLoop() {
  var sessionData =
    "&shandle=" +
    deviceInfos[selectedDeviceIndex].DeviceHandle +
    "&id=" +
    pageID;
  ...
  jQuery.ajax({
    ...
    success: function (msg) {
      ...
      if (msg) {
        ...
        var cookieStr =
          "username=" + msg.sessionId + "; expires=" + expires.toUTCString();
...
function LoadImage() {
  ...
  var src = urlStr + "/img/CaptureImg.bmp?dummy=" + Math.random() + sessionData;
  ...
  fetch(src).then((res) => {
    ...
    let file = new File([blob], "finget-print-" + Date.now() + ".jpg", metadata)

    UnInit(() => {
      DeletePage(() => {
        //window.opener.onFingerConfirm(file)
...
function GetImageBuffer() {
  ...
  var _dataType = document.getElementById("DDb_Tftype").value;
  if (_dataType == 3 && $("#chb_varsize").is(":checked")) {
    _dataType = 4;
  }
  jQuery.ajax({
    type: "GET",
    ...
    data: {
      ...
      id: pageID,
      fileType: _dataType,
      width: document.getElementById("number_width").value,
...
function PreviewOnChecked() {
  ...
  if (IEflag == 0) {
    ...
    document
      .getElementById("Fpimg")
      .setAttribute("src", imgUrl + Math.random() + sessionData);
  } else {
    $("#Cb_PreviewOn").attr("checked", 0);
...
function Enroll() {
  ...
  if (cb_SelectTemplateOpt == 1)
    Toast(
      "Placed your Finger on Camera(4 times, 2 template will be selected)",
      gToastTimeout,
    );
  else Toast("Placed your Finger on Camera", gToastTimeout);
...
function deleteUserInfo(delUserIndex) {
  ...
  for (var i = 0; i < numOfEnrolledUser; i++) {
    if (i < delUserIndex) insertUserInfo(i, userIDs[i], userTemps[i]);
    else if (i > delUserIndex) insertUserInfo(i - 1, userIDs[i], userTemps[i]);
  }
}
